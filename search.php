              ...
              //GET BOOKS With this author
                   $book_by_author_search="SELECT * FROM books WHERE author_id={$author['id']} AND uploaded_by={$user_id}";
                        ...
                        if($book_by_author_result){
                            //foreach book_by_author by that author, GET books in user_id Hippocampus by that user AND user_id
...
//=======================================================
//            
//      //SEARCH TITLES BY IN USER_ID Hippocampus
//            
//=======================================================
    
        if(isset($_GET['Hippocampus'])){
        $user_id=$_GET['Hippocampus'];
        ...
        if(preg_match("/^[_a-zA-Z0-9- ]+$/", $_POST['query'])){
                ...
                $username= $find_user['username'];
                                ...
                                ?>
                <h4><a href="profile.php?user=<?php echo $user_id; ?>"> <?php echo $username; ?></a>'s Hippocampus </h4>    
                    <form class="center search h4_search"  action="search.php?Hippocampus=<?php echo $user_id; ?>" method="post">
                        <input class="" type="text" name="query" value="" placeholder="Search this Hippocampus..." />    
                        <input type="submit" name="submit" value="&#xf002;" />
                    </form> <br/>
                    <a href="Hippocampus.php?user=<?php echo $user_id; ?>">&laquo; View All</a><br/>
        ...
        foreach($string_array as $word){ 
            ...
            $Hippocampus_search="SELECT * FROM bookshelves WHERE user_id={$user_id}";
            $Hippocampus_result=mysqli_query($connection, $Hippocampus_search);
            if($Hippocampus_result){
                foreach($Hippocampus_result as $Hippocampus_book){
                    
                    
                    //SEARCH BOOK DATA FOR EACH BOOK IN Hippocampus
                    $book_search="SELECT * FROM books WHERE id={$Hippocampus_book['book_id']} AND title LIKE '%" . $query_string ."%'";
                  ...
                  //search authors who's BOOK IDs are in Hippocampus BY NAME LIKE QUERY 
                    
                    $author_search="SELECT * FROM books WHERE id={$Hippocampus_book['book_id']}";
                    $author_result=mysqli_query($connection, $author_search);
                    if($author_result){ 
                        foreach($author_result as $author){
...
//                                    echo "Author found.<br/>";
                        ...
                        }//foreach book in Hippocampus
                    }//if books found in Hippocampus
                    
            }//end foreach book in Hippocampus
                    
                }//end limit books to Hippocampus
                
 
 
        }//end foreach word queried
    }// END PREG MATCH    
 }//END IF ISSET USER Hippocampus
